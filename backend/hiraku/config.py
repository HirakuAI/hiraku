...
from hiraku.utils.redis import get_redis_connection
...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config(HIRAKU_DIR / "alembic.ini")

        # Set the script location dynamically
        migrations_path = HIRAKU_DIR / "migrations"
...
STATIC_DIR = Path(os.getenv("STATIC_DIR", HIRAKU_DIR / "static")).resolve()
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (Hiraku AI + Ollama)
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get(
    "ELASTICSEARCH_INDEX_PREFIX", "hiraku_collections"
)
# Pgvector
